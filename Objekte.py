...
class cls_Enemy(object):
    ...
    def damage_and_death_anim(self, screen, damage_or_death, enemy_tile_Art):
        ...
        if enemy_tile_Art==GRASS:
            ...
            second = pygame.image.load('./resources/images/ressourcen/highgrasstexture.png').convert()
        elif enemy_tile_Art==DIRT:
            second = pygame.image.load('./resources/images/ressourcen/dirttexture.png').convert()
        elif enemy_tile_Art==WATER:
            second = pygame.image.load('./resources/images/ressourcen/watertexture.png').convert()
        ...
        for i in range (m):
            ...
            deathAnim.blit(screen, (self.Position[0]*Weltkarte.TILESIZE,self.Position[1]*Weltkarte.TILESIZE))
            pygame.display.update()
            mainClock.tick(30)
            i -= 1

        if damage_or_death=="death":
            screen.blit(second, (self.Position[0]*Weltkarte.TILESIZE, self.Position[1]*Weltkarte.TILESIZE))
        elif damage_or_death=="damage":
            screen.blit(first, (self.Position[0] * Weltkarte.TILESIZE, self.Position[1] * Weltkarte.TILESIZE))
    def generate_Enemy(self):
        ...
        Position=[int((Wahrscheinlichkeiten.wuerfel(Weltkarte.MAPWIDTH-1))),int((Wahrscheinlichkeiten.wuerfel(Weltkarte.MAPHEIGHT-1)))]
